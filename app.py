...
def load_user_data(username: str) -> tuple[list, ChatMessageHistory]:
    ...
    if history_file.exists():
        try:
            ...
            st.warning(f"history failed for {username} starting with a fresh history")
        except Exception as e:
            st.error(f"error loading history for {username}: {e}.")
...
if username_input != st.session_state.username:
    if st.session_state.username: #for previous username
...
if not username:
    st.info("enter username pookiee")
    st.stop()
